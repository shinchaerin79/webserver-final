...
<%
	try {
    	...
    	while (rs.next()) {
        	...
        	if (isPaid) btnClass = "btn-secondary"; // 결제 완료된 좌석 색
        	
        	else if (selected_user != null) {
            	if (selected_user.equals(userId)) btnClass = "btn-warning"; // 본인이 선택한 좌석 색
            	
            	else btnClass = "btn-secondary"; // 다른 사람이 선택한 좌석 색
        	}
...
%>
        	<button class="btn seat-button <%= btnClass %>" 
            	<%= isDisabled ? "disabled" : "" %>
            	onclick="selectSeat(this, <%= seat_number %>, '<%= userId %>', <%= scheduleId %>)">
